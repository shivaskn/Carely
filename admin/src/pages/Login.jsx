...
const Login = () => {
  ...
  const handelSubmit = async (event) => {
    ...
    try {
        ...
        const {data} = await axios.post('https://carely-23w9.onrender.com/api/admin/login',input)
        if(data.success){
            ...
            const {data} = await axios.post("https://carely-23w9.onrender.com/api/doctor/login", input)
            if(data.success){
              {/* So inga vanthu nammaloda token ha localstrong la save or set pannurom*/}
